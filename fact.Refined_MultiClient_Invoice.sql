...
/*
	...
	select top 10 * from fact.Refined_MultiClient_Invoice
...
IF OBJECT_ID(N'fact.Refined_MultiClient_Invoice') IS NOT NULL
    DROP view fact.Refined_MultiClient_Invoice
GO
CREATE VIEW fact.Refined_MultiClient_Invoice
AS
...
select
		...
		TransType = null,
		TransTypeTrust = null,
		TransTypeConfidence = null,
		TransTypeAuthority = null,
		TransTypeDtmUtc = null,
		--Flow components
		BusinessFlow = null,
		BusinessFlowTrust = null, 
		BusinessFlowConfidence = null, 
		BusinessFlowAuthority = null, 
		BusinessFlowDtmUtc = null,
		ExecPath = null,
		ExecPathTrust = null,
		ExecPathConfidence = null,
		ExecPathAuthority = null,
		ExecPathDtmUtc = null,
		...
		BLNG_FormattedPlace = case when bt.UntNId is not null then bt.FormattedPlace
								else 
								isnull([AL_CITY_BLNG] + ',', '') + 
								isnull([AL_STATE_PROV_BLNG] + ',', '') + 								 
								isnull([AL_CNTRY_CODE_BLNG], '') 
		...
		REMIT_FormattedPlace = case when br.UntNId is not null then br.FormattedPlace
								else 
								isnull([AL_CITY_REMIT] + ',', '') + 
								isnull([AL_STATE_PROV_REMIT] + ',', '') + 								 
								isnull([AL_CNTRY_CODE_REMIT], '') 
		...
		ACCT_NUM_VEND_BLNG,
		AcctNumVendBlngTrust = null,
		AcctNumVendBlngConfidence = null,
		AcctNumVendBlngAuthority = null,
		AcctNumVendBlngValueSource = null,
    ...
    FROM dbo.OutInvoice inv with(nolock)     
    LEFT JOIN dbo.OutInvoice_Ext ext with(nolock) on inv.Inv_ID = ext.Inv_ID
