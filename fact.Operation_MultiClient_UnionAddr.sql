...
/*
	select * from fact.Operation_MultiClient_UnionAllAddr
	order by UNTNID, UNTTYPEID, AddrTypeId
*/

IF OBJECT_ID(N'fact.Operation_MultiClient_UnionAddr') IS NOT NULL
    DROP view fact.Operation_MultiClient_UnionAddr
GO
CREATE VIEW fact.Operation_MultiClient_UnionAddr
AS
with AddrRaw_CTE(UntNId, UntTypeId, AddrTypeId, AddrType, InstNum,
	...
	/*select 
		UntNId,UntTypeId,AddrTypeId,
		AddrType = min(AddrType),InstNum,
		Street1, Street2, PortStation,City,
		StateProv,PostalCode,CountryCode,FormattedAddress,FormattedPlacePostalCode
	from AddrRaw_CTE
	GROUP BY 
		UntNId,UntTypeId,AddrTypeId,InstNum,
		Street1, Street2, PortStation,City,
		StateProv,PostalCode,CountryCode,FormattedAddress,FormattedPlacePostalCode
	*/
	select * from AddrRaw_CTE
	;
